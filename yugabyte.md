...
gcloud dns record-sets delete yugabyte.global.demo.gcp.red-chesterfield.com --type=A --zone=${global_base_domain_no_dots}
gcloud dns record-sets create yugabyte.global.demo.gcp.red-chesterfield.com --rrdatas=${IPs} --type=A --ttl=60 --zone=${global_base_domain_no_dots}
```

### Create global load balancer -- gcp only

DNS->global VIP->forwarding rule->target tcp proxy->backend-service->[neg]->[ne]
                                                    health-check

```shell
export service_name=yugabyte

# create global address
gcloud compute addresses create ${service_name}-global-ipv4 --ip-version=IPV4 --global
export service_global_ip=$(gcloud --format json compute addresses describe ${service_name}-global-ipv4 --global | jq -r .address)

# create dns entry
export cluster_base_domain=$(oc --context ${control_cluster} get dns cluster -o jsonpath='{.spec.baseDomain}')
export global_base_domain=global.${cluster_base_domain#*.}
export global_base_domain_no_dots=$(echo ${global_base_domain} | tr '.' '-')
gcloud dns record-sets create ${service_name}.${global_base_domain} --rrdatas=${service_global_ip} --type=A -z ${global_base_domain_no_dots}
...
# create health check 
gcloud compute health-checks create https ${service_name}-health-check --host ${service_name}.${global_base_domain} --request-path / --use-serving-port --global

# create backend service
gcloud compute backend-services create ${service_name}-backend-service --global --health-checks ${service_name}-health-check --load-balancing-scheme EXTERNAL --protocol tcp --global-health-checks
...
for cluster in ${cluster1} ${cluster2} ${cluster3}; do
  ...
  for zone in $(gcloud --format json compute zones list --filter region=${region} | jq -r .[].name); do
    gcloud compute network-endpoint-groups create ${service_name}-${zone} --network=${network} --subnet=${subnet} --default-port=443  --zone=${zone}
    for instance in $(gcloud --format json compute instances list --zones ${zone} --filter="labels.kubernetes-io-cluster-${infrastructureName}=owned" | jq -r .[].name); do
      gcloud compute network-endpoint-groups update ${service_name}-${zone} --zone=${zone} --add-endpoint "instance=${instance},port=${port}"
    done
    gcloud compute backend-services add-backend ${service_name}-backend-service --global --network-endpoint-group=${service_name}-${zone} --network-endpoint-group-zone=${zone} --balancing-mode connection --max-connections-per-endpoint 100
  done
done

# create target https proxy
gcloud compute target-tcp-proxies create ${service_name}-target-tcp-proxy --backend-service ${service_name}-backend-service

# create forwarding rule

gcloud compute forwarding-rules create ${service_name}-forwarding-rule --ip-protocol tcp --address ${service_name}-global-ipv4 --load-balancing-scheme external --ports 443 --global --target-tcp-proxy ${service_name}-target-tcp-proxy --global-address
```
